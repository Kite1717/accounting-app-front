import React, { useMemo } from "react";
import { useChequesUIContext } from "../ChequesUIContext";

export function ChequesGrouping() {
  // Cheques UI Context
  const chequesUIContext = useChequesUIContext();
  const chequesUIProps = useMemo(() => {
    return {
      ids: chequesUIContext.ids,
      setIds: chequesUIContext.setIds,
      openDeleteChequesDialog: chequesUIContext.openDeleteChequesDialog,
      openFetchChequesDialog: chequesUIContext.openFetchChequesDialog,
      openUpdateChequesStatusDialog:
        chequesUIContext.openUpdateChequesStatusDialog,
    };
  }, [chequesUIContext]);

  return (
    <div className="form">
      <div className="row align-items-center form-group-actions margin-top-20 margin-bottom-20">
        <div className="col-xl-12">
          <div className="form-group form-group-inline">
            <div className="form-label form-label-no-wrap">
              <label className="font-bold font-danger">
                <span>
                  Selected records count: <b>{chequesUIProps.ids.length}</b>
                </span>
              </label>
            </div>
            <div>
              <button
                type="button"
                className="btn btn-danger font-weight-bolder font-size-sm"
                onClick={chequesUIProps.openDeleteChequesDialog}
              >
                <i className="fa fa-trash"></i> Delete All
              </button>
              &nbsp;
              <button
                type="button"
                className="btn btn-light-primary font-weight-bolder font-size-sm"
                onClick={chequesUIProps.openFetchChequesDialog}
              >
                <i className="fa fa-stream"></i> Fetch Selected
              </button>
              &nbsp;
              <button
                type="button"
                className="btn btn-light-primary font-weight-bolder font-size-sm"
                onClick={chequesUIProps.openUpdateChequesStatusDialog}
              >
                <i className="fa fa-sync-alt"></i> Update Status
              </button>
            </div>
          </div>
        </div>
      </div>
    </div>
  );
}
