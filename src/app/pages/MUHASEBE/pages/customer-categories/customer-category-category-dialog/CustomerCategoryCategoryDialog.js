...
export function CustomerCategoryCategoryDialog({ show, onHide }) {
  ...
  const { isLoading, superCategory } = useSelector(
    (state) => ({
      ...
      superCategory: state.customerCategories.superCategory,
    }),
    shallowEqual
  );
  ...
  useEffect(() => {
    if (show) {
      dispatch(actions.fetchSuperCustomerCategoryCategory());
      setInit({
        title: "",
        superCategoryId: "1",
      });
