...
import { BankStatusCssClasses, BankStatusTitles } from "../BanksUIHelpers";
...
export function BanksUpdateStateDialog({ show, onHide }) {
  // Banks UI Context
  ...
  const { banks, isLoading } = useSelector(
    (state) => ({
      banks: selectedBanks(state.banks.entities, banksUIProps.ids),
                ...
                <td>
                  <span
                    className={`label label-lg label-light-${
                      BankStatusCssClasses[bank.status]
                    } label-inline`}
                  >
                    {" "}
                    {BankStatusTitles[bank.status]}
                  </span>
                </td>
                <td>
                  <span className="ml-3">
